...
class AgentAttention(nn.Module):
    ...
    def flops(self, n):
        ...
        # n:  window内patch数量, typical = window_size[0]*window_size[1] (e.g. 49 for 7x7)
        # A:  agent_num
        # C:  dim
        # H:  num_heads
        # head_dim = C // H
        ...
        # 2) Agent -> Global 的双矩阵乘
        #    (agent_tokens @ k^T) + (attn @ v)
        #    agent_tokens & k,v 形状约 (H, A, head_dim) × (H, n, head_dim) => O(H * A * n * head_dim) × 2
        #    => 2 * H * A * n * (C/H) = 2 * A * n * C
        flops_agent_global = 2 * H * A * n * head_dim
        flops += flops_agent_global

        # 3) Global -> Agent 的双矩阵乘
        #    (q @ agent_tokens^T) + (attn @ agent_v)
        #    形状约 (H, n, head_dim) × (H, A, head_dim) => O(H * n * A * head_dim) × 2
        #    => 2 * A * n * C
        flops_global_agent = 2 * H * n * A * head_dim
        flops += flops_global_agent

        # 4) Depthwise Conv (3x3, groups=C)
        #    输入形状 ~ (C, sqrt(n), sqrt(n)), kernel大小=3x3
        #    => 9 * n * C
        flops_dwconv = 9 * n * C
        flops += flops_dwconv
...
#         # self.reduce = PatchReduce2D_sim(dim, dim_out, dim_scale=2)
#         self.input_resolution = input_resolution
#         self.dim=dim
#         self.out_dim=out_dim
#         self.reduction = nn.Sequential(
#             nn.Conv2d(dim, out_dim, 2, 2,bias=False),
#         )
...
#         # print(x.size())
#         x = self.reduction(x).flatten(2).transpose(1, 2)
#         # print(x.size())
#         return x
...
#                 nn.ConvTranspose2d(dim, out_dim, kernel_size=2, stride=2, padding=0, bias=False),
#             )
